...
export const StoreList = ({ items, userRole, onEdit, onDelete }) => {
    if (items.length === 0) return (
        <div className={styles.empty}>
            <img src='empty.png' alt="" />
            <p className={styles.empty__text}>No store products found</p>
            <span className={styles.empty__sub}>Try adjusting filters</span>
        </div>
    );
    ...
    return (
        <div className={styles.list}>
            <div className={styles.list__head}>
                ...
                <span>Quantity</span>
                <span>Type</span>
                {userRole === 'manager' && <span></span>}
            </div>

            {items.map((sp, i) => {
                const vatAmt = +(sp.price * 0.2).toFixed(2);
                return (
                    <div
                        key={sp.upc}
                        className={`${styles.row} ${sp.isPromo ? styles['row--promo'] : ''}`}
                        style={{ animationDelay: `${i * 30}ms` }}
                    >
                        <span className={styles.row__upc}>{sp.upc}</span>
                        <span className={styles.row__name}>{sp.productName}</span>
                        <span className={styles.row__muted}>{sp.categoryName}</span>
                        <span className={styles.row__price}>{sp.price.toFixed(2)} ₴</span>
                        <span className={styles.row__vat}>{vatAmt.toFixed(2)} ₴</span>
                        <span className={styles.row__qty}>{sp.quantity} pcs</span>

                        <span className={sp.isPromo
                            ? styles['row__badge--promo']
                            : styles['row__badge--regular']}
                        >
                            {sp.isPromo ? 'Promo −20%' : 'Regular'}
                        </span>

                        {userRole === 'manager' && (
                            <div className={styles.row__actions}>
                                <button className={styles.row__edit}
                                    onClick={() => onEdit(sp)} title="Edit">
                                ...
                                <button className={styles.row__delete} onClick={() => onDelete(sp.upc)} title="Delete">

...
const POSITION_LABELS = { manager: 'Manager', cashier: 'Cashier' };

export const EmployeesList = ({ employees, onEdit, onDelete }) => {
    if (employees.length === 0) return (
        <div className={styles.empty}>
            <img src='empty.png' alt="" />
            <p className={styles.empty__text}>No employees found</p>
    ...
    return (
        <div className={styles.list}>
            ...
            {employees.map((emp, i) => (
                ...
                >
                    <span className={styles.row__id}>{emp.id}</span>

                    <span className={styles.row__name}>
                        {emp.lastName} {emp.firstName} {emp.patronym}
                    </span>

                    <span className={styles[`row__badge--${emp.position}`]}>
                        {POSITION_LABELS[emp.position]}
                    </span>

                    <span className={styles.row__phone}>{emp.phone}</span>

                    <span className={styles.row__address}>{emp.address}</span>

                    <span className={styles.row__date}>{emp.startDate}</span>

                    <span className={styles.row__date}>{emp.birthDate}</span>

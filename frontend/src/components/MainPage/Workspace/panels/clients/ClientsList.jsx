...
export const ClientsList = ({ clients, userRole, onEdit, onDelete }) => {
    if (clients.length === 0) return (
        <div className={styles.empty}>
            <img src='empty.png' alt="" />
            <p className={styles.empty__text}>No clients found</p>
    ...
    return (
        <div className={styles.list}>
            ...
            {clients.map((client, i) => (
                ...
                >
                    <span className={styles.row__id}>{client.cardId}</span>

                    <span className={styles.row__name}>
                        {client.lastName} {client.firstName} {client.patronym}
                    </span>

                    <span className={styles.row__phone}>{client.phone}</span>

                    <span className={styles.row__address}>{client.address}</span>

                    <span className={styles.row__badge}>
                        {client.discount}%
                    </span>
                    ...
                    <div className={styles.row__actions}>
                        ...
                        {userRole === 'manager' && (
                            <button
                                ...
                                onClick={() => onDelete(client.cardId)}
                                title="Delete"
                            >

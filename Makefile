...
env:
	...
	@test -f auth-service/.env || ( \
		...
		echo "BUSINESS_SERVICE_ADDR=business-service:2433" >> auth-service/.env && \
	...
	@test -f business-service/.env || ( \
		echo "GRPC_PORT=2433" > business-service/.env && \
		echo "DATABASE_URL=postgres://postgres:cisco@postgres:5432/zlahoda_data?sslmode=disable" >> business-service/.env \
	)

	@echo "=== Generating .env for API Gateway ==="
	@test -f api-gateway/.env || ( \
		echo "PORT=8080" > api-gateway/.env && \
		echo "BUSINESS_SERVICE_ADDR=business-service:2433" >> api-gateway/.env && \
		echo "AUTH_SERVICE_ADDR=auth-service:3131" >> api-gateway/.env \
	)
...
front-dbg-down:
	@echo "Stopping frontend debug infrastructure..."
	docker compose -f docker/test/docker-compose.front-debug.yml down
